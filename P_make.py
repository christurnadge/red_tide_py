...
def P_make(f, S, F, varargin):
    if nargin==3:
        ...
        print('3 or 5 or 6 inputs expected, given '+str(nargin)+'. See documentation.')
    ...
    F_full = [F, F_unmodeled.T]
    F_full = np.unique(F_full)
    if strcmp(INTERP_METHOD, 'loglinear'):
        Sxx_Prior = np.exp(np.interp1([f;(f_Ny+df)], np.log([S;S(end)]), F_full, 'linear'))
    else:
        Sxx_Prior = np.interp1([f;(f_Ny+df)], [S;S(end)], F_full, INTERP_METHOD)
    Sxx_Prior[Sxx_Prior<=0] = np.min(Sxx_Prior[Sxx_Prior>0])
    if np.isinf(S[1]):
        print('Your averaged spectral estimates begin with a non-finite value, which absolutely should not be the case.')
    ...
    P_diag[0:1:2*len(Sxx_ModelPrior)] = Sxx_ModelPrior
    P_diag[1:1:2*len(Sxx_ModelPrior)] = Sxx_ModelPrior
