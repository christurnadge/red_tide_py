    ...
    if nargin==9:
        ...
        if iscell(tide_f):
            ...
            for i in range(len(sideband_centers)):
                for k in range(length(F_cusp_boolean)):
                    if np.isempty(sideband_centers):
                        F_user_defined_center = []
                    elif isinstance(sideband_centers[i], str):
        ...
        F = [F_lowNO, F_lowO, F_inertial, F_cusps, F_cuspcenters, F_tidal].T
        F = np.unique(F)
        if df_sidebands==0 || ~np.isfinite(df_sidebands):
            df_sidebands = df
        diff_F = np.diff(F)
        if np.isempty(F_lowNO):
            if np.sum(diff_F<0.99*df_sidebands):
                print('In your F vector, you have a frequency difference less than 1/('+
                      str((1./df_sidebands))+' hr) = df_sidebands (or the fundamental frequency '+
                      ' df = 1/(record length) if df_sidebands was given as 0, i.e. skipped). '+
                      ' This could lead to very high uncertainty at this and nearby frequencies.'+
                      ' The corresponding F indices are:')
                F_Indices = range(len(F))
                print(F_Indices[diff_F<0.99*df_sidebands])
        else:
            if np.sum(diff_F<0.99*df_NO):
                print('In your F vector, you have a frequency difference less than the'+
                      ' lowest you prescribed in df_NO = 1/('+str((1./df_NO))+' hr). This could lead'+
                      ' to very high uncertainty at this and nearby frequencies. The corresponding'+
                      ' F indices are:')
                F_Indices = range(len(F))
        ...
        NOAA_HARM_CONST = [1       f_m2  , # M2
                           ...
                           36      f_m8  , # M8
                           37      f_ms4]  # MS4
        F = np.nan(varargin{1}, 1)
        for i in range(varargin{1}):
